...
class TestCompletionArtifactIntegration:
    ...
    def test_flow_completion_generates_artifact(self):
        ...
        response = self.client.post("/runs", json={"flow_id": "auto-complete-flow", "user_id": "test-user"})
        ...
        artifact_response = self.client.get(f"/runs/{run_id}/artifact")
    ...
    def test_flow_completion_updates_status_correctly(self):
        ...
        # Start a flow
        response = self.client.post("/runs", json={"flow_id": "quick-flow", "user_id": "test-user"})
        assert response.status_code == 201
    ...
    def test_completed_flow_artifact_persistence(self):
        ...
        time.sleep(1)
        artifact_response2 = self.client.get(f"/runs/{run_id}/artifact")
    ...
    def test_failed_flow_does_not_generate_artifact(self):
        ...
        response = self.client.post("/runs", json={"flow_id": "failing-flow", "user_id": "test-user"})
        assert response.status_code == 201
    ...
    def test_artifact_content_type_based_on_flow_type(self):
        ...
        response = self.client.post("/runs", json={"flow_id": "pdf-generation-flow", "user_id": "test-user"})
        assert response.status_code == 201
        ...
        if "content-type" in artifact_response.headers:
            assert "pdf" in artifact_response.headers["content-type"].lower() or \
                   "application" in artifact_response.headers["content-type"].lower()
    ...
    def test_large_artifact_handling(self):
        ...
        response = self.client.post("/runs", json={"flow_id": "large-output-flow", "user_id": "test-user"})
        assert response.status_code == 201
    ...
    def _wait_for_status(self, run_id: str, target_status: str, timeout: int = 30):
        ...
        pytest.fail(f"Run {run_id} did not reach status {target_status}, current status: {current_status}")
