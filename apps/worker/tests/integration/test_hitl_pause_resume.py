...
class TestHITLPauseResumeIntegration:
    ...
    def test_hitl_flow_pauses_on_user_input_required(self):
        ...
        response = self.client.post("/runs", json={"flow_id": "hitl-flow", "user_id": "test-user"})
    ...
    def test_hitl_resume_after_user_action(self):
        ...
        resume_response = self.client.post(f"/runs/{run_id}/continue", json={"action": "click_button"})
    ...
    def test_hitl_multiple_pause_resume_cycles(self):
        ...
        response = self.client.post("/runs", json={"flow_id": "multi-hitl-flow", "user_id": "test-user"})
        assert response.status_code == 201
        ...
        # First pause
        self._wait_for_status(run_id, "awaiting_input")
        resume_response = self.client.post(f"/runs/{run_id}/continue", json={"action": "fill_form"})
        assert resume_response.status_code == 200

        # Second pause
        self._wait_for_status(run_id, "awaiting_input")
        resume_response = self.client.post(f"/runs/{run_id}/continue", json={"action": "submit_form"})
        assert resume_response.status_code == 200
    ...
    def test_hitl_resume_with_invalid_action(self):
        ...
        resume_response = self.client.post(f"/runs/{run_id}/continue", json={"action": "invalid_action"})
    ...
    def test_hitl_resume_preserves_session_state(self):
        ...
        # Resume
        resume_response = self.client.post(f"/runs/{run_id}/continue", json={"action": "proceed"})
        assert resume_response.status_code == 200

        # Verify session URL is preserved
        get_response_after = self.client.get(f"/runs/{run_id}")
